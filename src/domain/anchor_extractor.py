...
def extract_anchors(query: str, anchors_cfg: dict, aliases_cfg: dict) -> dict:
    ...
    weak_candidates = (
        weak_cfg.get("intent_terms", []) + 
        weak_cfg.get("doc_terms", [])
    )
    ...
    for cand in weak_candidates:
        if cand in tokens: # Exact token match often better for common words, but mandate implies substring logic for aliases.
