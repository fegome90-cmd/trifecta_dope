...
class ContextService:
    ...
    def search(self, query: str, k: int = 5, doc_filter: Optional[str] = None) -> SearchResult:
        ...
        for entry in pack.index:
            ...
            if "skill" in entry.id and any(kw in query_words for kw in ["regla", "comando", "cómo", "rule", "protocol"]):
                score += 0.5
            if "agent" in entry.id and any(kw in query_words for kw in ["stack", "código", "tech", "implement", "debug", "fix"]):
                score += 1.0
            if "session" in entry.id and any(kw in query_words for kw in ["pasos", "checklist", "runbook", "handoff", "history", "log"]):
                score += 0.8
    ...
    def _skeletonize(self, text: str) -> str:
        ...
        for line in text.splitlines():
            ...
            if in_code_block and len(skeleton_lines) > 0 and skeleton_lines[-1].strip().startswith("```"):
                 if any(kw in line for kw in ["def ", "class ", "interface ", "function ", "const ", "var "]):
