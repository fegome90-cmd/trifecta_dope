...
| SH-003 | cerrado canónico (gobernanza) | operational / governance | alta | wrapper + cadena runtime gobernada | Existe una ruta oficial de promoción/verificación con receipt para `skill-hub` y `skill-hub-cards`, `skill_hub_info_card.py` quedó fuera de la cadena directa y ADR-004 ya fue ratificado | `scripts/skill-hub-runtime`; receipt `~/.local/share/trifecta/receipts/skill-hub-runtime.json`; `~/.local/bin/skill-hub` y `~/.local/bin/skill-hub-cards` promovidos; verify actual `verification ok` | La gobernanza documental de SH-003 queda cerrada sobre la cadena runtime auditada vigente; la deuda fuera de alcance sigue en findings separados | Mantener verificación fail-closed para la instalación vigente y tratar los frentes separados fuera de SH-003 en sus findings correspondientes |
...
- ADR-004 quedó ratificado en `ACCEPTED`.
- SH-003 queda cerrado canónicamente en gobernanza porque sigue existiendo promoción oficial verificable para la instalación vigente y su cadena runtime gobernada.
