...
def test_render_skill_contains_session_evidence_persistence() -> None:
    ...
    # Updated to match compacted template
    assert "Session Evidence Persistence" in output
    assert "Persist intention" in output
    assert "Sync context" in output
    assert "Execute" in output
    assert "Log result" in output
...
def test_no_forbidden_strings_in_templates() -> None:
    ...
    for forbidden in forbidden_strings:
        ...
        assert forbidden not in session_output, (
            f"Found forbidden string '{forbidden}' in session.md"
        )
