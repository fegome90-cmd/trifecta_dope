...
class TestDeduplicationContract:
    ...
    def test_skill_md_indexed_only_once_in_context_pack(self):
        ...
        skill_chunks = [
            chunk for chunk in pack.get("chunks", [])
            if chunk.get("doc", "").startswith("skill") or 
               chunk.get("doc", "").startswith("ref:skill")
        ]
    ...
    def test_nested_skill_md_is_NOT_excluded(self, tmp_path):
        ...
        assert len(ref_skill_chunks) == 1, "Nested library/python/skill.md must be indexed as reference"
