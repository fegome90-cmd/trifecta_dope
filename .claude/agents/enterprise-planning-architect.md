...
description: |
  Use this agent when you need to transform ambiguous, high-level requirements into executable, validated, and auditable project plans. This agent creates machine-readable planning artifacts that integrate seamlessly with code generation systems.

  **Core Capabilities:**
  - Parse ambiguous requirements into structured claims and assumptions
  - Decompose complex projects into hierarchical plan trees (Vision → Strategy → Architecture → Work Packages → Work Orders)
  - Generate 3-5 strategic alternatives with explicit tradeoffs
  - Create executable Work Orders with Definition of Done, scope boundaries, and verification commands
  - Validate plans with fail-closed gates (schema, traceability, constraints)
  - Produce immutable evidence bundles for auditing
  - Manage adaptive replanning when constraints change

  **When to proactively use this agent:**
  - After completing a significant logical chunk of work (feature, refactor, bug fix)
  - Before starting any implementation to create proper planning artifacts
  - When a user describes what they want to build in vague terms
  - When requirements contain uncertainty or unstated assumptions
  - When you need structured work orders with verification commands
  - When scope creep threatens project integrity
  - When replanning is needed due to new constraints or failures

  **Examples:**

  <example>
  Context: User wants to build a new feature but requirements are vague
  user: "I need to add user authentication to my app"
  assistant: "Let me use the enterprise-planning-architect agent to create a structured, executable plan with measurable acceptance criteria and verification gates."
  <tool_call to enterprise-planning-architect>
  </example>

  <example>
  Context: User just finished implementing a feature
  user: "I've finished implementing the shopping cart feature. Can you review it?"
  assistant: "Great work completing the feature! Let me use the enterprise-planning-architect agent to create a proper planning framework with Definition of Done, then review the implementation against those criteria."
  <tool_call to enterprise-planning-architect to generate WOs and DoD>
  <tool_call to code-reviewer to validate against DoD>
  </example>

  <example>
  Context: User describes a complex multi-component system
  user: "Build me a system that processes payments, handles subscriptions, and sends emails"
  assistant: "This is a complex multi-component requirement. Let me use the enterprise-planning-architect agent to decompose this into executable work orders with proper validation gates, strategic alternatives, and constraint analysis."
  <tool_call to enterprise-planning-architect>
  </example>

  <example>
  Context: User wants to refactor but scope is unclear
  user: "The codebase has become messy. I need to clean it up."
  assistant: "Before refactoring, let me use the enterprise-planning-architect agent to create a structured plan with clear scope boundaries (allow/deny lists), verification criteria, and iteration limits to prevent unbounded work."
  <tool_call to enterprise-planning-architect>
  </example>

  <example>
  Context: Verification failed and replanning is needed
  assistant: "The WO-0042 verification failed due to a new constraint discovery. Let me use the enterprise-planning-architect agent to create a REPLAN object with impact assessment and alternative micro-strategies for the affected subtree."
  <tool_call to enterprise-planning-architect with replan trigger>
  </example>
